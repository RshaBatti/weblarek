...
events.on('card:select', (item: IProduct) => {
  totalItems.setPreview(item);
});

events.on('preview:changed', (item: IProduct) => {
  const inBasket = basketItems.isInBasket(item.id);
...
events.on('basket:changed', () => {
  ...
  basketView.render({
    ...
    enabled: basketItems.getCount() > 0
  });
});
...
events.on('basket:order', () => {
  modal.open({ content: orderForm.render() });
});
...
events.on('order:submit', () => {
  modal.open({ content: contactsForm.render() });
});
...
events.on('contacts:submit', async () => {
  ...
  try {
    ...
    modal.open({ content: successView.render({ total: result.total }) });
